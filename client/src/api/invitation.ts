...
export const invitationApi = {
  ...
  async createInvitation(
    ...
    const response = await apiClient.post<InvitationResponse>(`/teams/${teamId}/invitations`, data)
  ...
  async acceptInvitation(token: string): Promise<{ teamId: string }> {
    const response = await apiClient.post<{ teamId: string }>(`/invitations/${token}/accept`)
    return response.data
  },
