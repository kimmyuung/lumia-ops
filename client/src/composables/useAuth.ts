...
export function useAuth() {
  ...
  async function register(data: RegisterRequest) {
    ...
    try {
      ...
      error.value =
        apiError.response?.data?.message || apiError.message || '회원가입에 실패했습니다.'
  ...
  async function login(data: LoginRequest) {
    ...
    try {
      ...
      const message =
        apiError.response?.data?.message || apiError.message || '로그인에 실패했습니다.'
  ...
  async function verifyEmail(token: string) {
    ...
    try {
      ...
      const apiError = err as { response?: { data?: { message?: string } }; message?: string }
      error.value =
        apiError.response?.data?.message || apiError.message || '이메일 인증에 실패했습니다.'
      toast.error(error.value)
      return false
    } finally {
      isLoading.value = false
  ...
  async function setNickname(nickname: string) {
    ...
    try {
      ...
      const apiError = err as { response?: { data?: { message?: string } }; message?: string }
      error.value =
        apiError.response?.data?.message || apiError.message || '닉네임 설정에 실패했습니다.'
      toast.error(error.value)
      return false
    } finally {
      isLoading.value = false
  ...
  async function resendVerification(email: string) {
    ...
    try {
      ...
      const apiError = err as { response?: { data?: { message?: string } }; message?: string }
      error.value =
        apiError.response?.data?.message || apiError.message || '이메일 발송에 실패했습니다.'
      toast.error(error.value)
      return false
    } finally {
      isLoading.value = false
