 ...
 * Canvas를 PDF로 내보내기
 */
export function exportAsPDF(canvas: HTMLCanvasElement, title?: string, filename?: string): void {
 ...
 * Canvas를 PPTX로 내보내기
 */
export function exportAsPPTX(canvas: HTMLCanvasElement, title?: string, filename?: string): void {
  const imgData = canvas.toDataURL('image/png')
  const name = filename || `strategy-map-${Date.now()}`
