...
test.describe('팀 관리', () => {
    ...
    test('팀 생성 모달 열기', async ({ page }) => {
        ...
        if (await createButton.isVisible()) {
            ...
            // 모달이 열리는지 확인
            await expect(page.locator('[role="dialog"]').or(page.locator('.modal'))).toBeVisible({ timeout: 2000 })

            // 팀 이름 입력 필드 확인
            await expect(page.locator('input[name="name"]').or(page.locator('input').filter({ hasText: /이름/ }))).toBeVisible()
        }
    })
...
test.describe('팀 멤버 관리', () => {
    ...
    test('멤버 목록 표시', async ({ page }) => {
        ...
        // 멤버 목록 또는 멤버 섹션 확인
        const memberSection = page.locator('[data-testid="team-members"]')
